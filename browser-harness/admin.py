...
def _port(name=None):
    n = name or NAME
    return 62000 + int(hashlib.md5(n.encode()).hexdigest()[:3], 16) % 1000


def _paths(name=None):
    n = name or NAME
    return _port(name), str(_TMPDIR / f"bu-{n}.pid")


def _log_tail(name=None):
    p = _TMPDIR / f"bu-{name or NAME}.log"
    try:
...
def ensure_daemon(wait=60.0, name=None, env=None):
    ...
    while time.time() < deadline:
        ...
        time.sleep(0.2)
    msg = _log_tail(name)
    raise RuntimeError(msg or f"daemon {name or NAME} didn't come up -- check {_TMPDIR / f'bu-{name or NAME}.log'}")
